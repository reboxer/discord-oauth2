...
class OAuth extends RequestHandler {
	...
	*/
	...
	/**
	 * Request all the roles for user of a specific guild
	 * Requires the `guilds` scope
	 * @arg {String} access_token The user access token
	...
	*/
	 ...
	 * 
	 * @arg {Object} opts
	 * @arg {String} opts.userId The user ID of the user to query for roles within the guild
	 * @arg {String} opts.guildId The guild id to query for roles
	 * @arg {String?} opts.botToken The token of the bot used to authenticate, defaults to supplied token during instantiation
	 * @returns {Promise<Object[]>}
	*/
	getMemberRolesForGuild(opts) {
		return this.request("GET", `/guilds/${opts.guildId}/members/${opts.userId}`, undefined, {
			auth: {
				type: "Bot",
				creds: opts.botToken || this.token,
			},
			contentType: "application/json",
		});
	}
